...
class Encoder(nn.Module):
    def __init__(self, inplanes, image_size, widen=1):
        super(Encoder, self).__init__()

        self.image_size = image_size

        self.encoder = nn.Sequential(
            nn.Conv2d(3, int(12 * widen), kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(int(12 * widen)),
            nn.ReLU(),
            nn.Conv2d(int(12 * widen), inplanes, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(int(inplanes)),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d((image_size, image_size)),
        )

    def forward(self, image_ori):
        return self.encoder(image_ori)


class ResEncoder(nn.Module):
    def __init__(self, inplanes, outplanes, out_size, widen=1):
        super(ResEncoder, self).__init__()

        self.encoder = nn.Sequential(
            nn.Conv2d(inplanes, int(outplanes), kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(int(outplanes)),
            nn.ReLU(),
